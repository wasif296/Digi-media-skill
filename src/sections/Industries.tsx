    ...
    <Box py={120} style={{ background: '#020408', position: 'relative', overflow: 'hidden' }}>
      ...
      <Container size="lg">
        <Stack align="center" mb={100} gap="xs">
          ...
          <Title order={2} ta="center" style={{ color: 'white', fontSize: 'clamp(2.5rem, 5vw, 4rem)', fontWeight: 900, letterSpacing: '-1px' }}>
            Deep Expertise in <span style={{ color: '#2DD4BF' }}>Your Industry</span>
          </Title>
              ...
              <Box key={item.id} mb={{ base: 80, md: 120 }}>
                ...
                >
                  <Box style={{ flex: 1, paddingRight: isTextLeft ? '60px' : '0', paddingLeft: isTextLeft ? '0' : '60px' }}>
                    ...
                    >
                      <Stack align={isTextLeft ? 'flex-end' : 'flex-start'} gap="md" style={{ textAlign: isTextLeft ? 'right' : 'left' } as any}>
                        <Box style={{ 
                        ...
                        <Title order={3} style={{ color: 'white', fontSize: '1.8rem', fontWeight: 800 }}>{item.title}</Title>
                        <Text c="dimmed" size="md" style={{ lineHeight: 1.6, maxWidth: '450px' }}>
                          {item.desc}
                        </Text>
                        ...
                        <Group gap="sm">
                          {item.stats.map(stat => (
                            <Box key={stat} style={{ 
                              padding: '5px 12px', borderRadius: '50px', border: '1px solid rgba(45, 212, 191, 0.2)',
                              background: 'rgba(45, 212, 191, 0.05)'
                            }}>
                              <Text size="10px" fw={800} style={{ color: '#2DD4BF', textTransform: 'uppercase' }}>{stat}</Text>
                            </Box>
                          ))}
                        </Group>
                      </Stack>
                  ...
                  <Box visibleFrom="md" style={{ zIndex: 5, width: '80px', display: 'flex', justifyContent: 'center' }}>
                    <Box style={{ 
                      ...
                      <Text size="10px" fw={900} style={{ color: '#2DD4BF' }}>{item.id}</Text>
                    </Box>
                  </Box>
                  ...
                  <Box style={{ flex: 1, paddingLeft: isTextLeft ? '60px' : '0', paddingRight: isTextLeft ? '0' : '60px' }}>
                    <motion.div
                      ...
                      transition={{ duration: 0.8 }}
                    >
                      <Box style={{ overflow: 'hidden', borderRadius: '24px', border: '1px solid rgba(255,255,255,0.05)', cursor: 'pointer' }}>
